...
export const get = query({args: {}, handler: async (ctx, args) => {
  ...
  const conversationWithDetails = await Promise.all(conversations.map(async (conversation, index) => {
    ...
    const unseenMessages = await ctx.db.query("messages").withIndex("by_conversationId", (q) => q.eq("conversationId", conversation._id)
).filter((q) => q.gt(q.field("_creationTime"), lastSeenMessageTime)).filter((q) => q.neq(q.field("senderId"), currentUser._id)).collect();
...
const getLastMessageDetails = async ({ctx, id} : {ctx: QueryCtx | MutationCtx; id: Id<"messages"> | undefined})=> {
    ...
    const message = await await ctx.db.get(id);
